 ...
 * `then()`. A subprocess's interior nodes are written directly on the subprocess class; its `inner()` returns
 * a leaf `Inner` scope holding only the interior's start elements — Java (JLS 8.1.3) forbids a nested `Inner`
 * enclosed by another `Inner`, which lexically nesting the interior would produce for a subprocess-in-subprocess.
 */
internal class JavaNavigationWriter {
    ...
    private fun addInnerScope(classBuilder: TypeSpec.Builder, node: NavigationNode, inner: NavigationGraph) {
        ...
        // The interior nodes live directly on the subprocess class, not nested inside `Inner`, so a nested
        // subprocess's own `Inner` is never enclosed by another `Inner` — a compile error in Java (JLS 8.1.3).
        inner.nodes.forEach { child -> classBuilder.addMethod(nodeAccessor(child.propertyName, child.objectName, static = false)) }
        inner.nodes.forEach { child -> classBuilder.addType(buildNavigationNode(child)) }
    }
    ...
    private fun buildInnerScope(node: NavigationNode, startNodes: List<NavigationNode>): TypeSpec {
        ...
        if (startNodes.isNotEmpty()) {
            ...
            innerBuilder.addMethod(thenMethod(static = false, override = true))
