...
1. Use Glob to find candidate files: `**/*ProcessApi*.kt` and `**/*ProcessApi*.java`, plus the shared definition files next to them (`ServiceTasks`, `Messages`, `Signals`, `Errors`, `Escalations` with `.kt` / `.java`).
...
- **Kotlin `object`**: Parse `const val NAME: String = "value"` lines within the top-level object and each nested `object` (Elements, Timers, Variables, CallActivities). The shared definition files hold one top-level `object` each (ServiceTasks, Messages, Signals, Errors, Escalations).
