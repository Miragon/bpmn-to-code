...
async function handleGenerate(e) {
    ...
    try {
        ...
        if (result.success) {
            ...
            state.libraryFiles = result.libraryFiles || [];
            state.runtimeDependency = result.runtimeDependency || null;
            renderResults(result.files);
            renderRuntimeDependency();
...
async function downloadAllAsZip() {
    ...
    try {
        ...
        if (state.runtimeDependency) {
            zip.file('README.md', buildReadmeContents());
        }
...
function buildReadmeContents() {
    const dep = state.runtimeDependency;
    if (!dep) return '';
    return [
        `# Generated by bpmn-to-code`,
        ``,
        `The Process API files in this archive import from \`${dep.group}.${dep.artifact.replace(/-/g, '.')}\`.`,
        `Add the \`bpmn-to-code-runtime\` artifact to your build to make them compile.`,
        ``,
        `## Gradle`,
        ``,
        '```kotlin',
...
function renderRuntimeDependency() {
    const card = document.getElementById('runtime-dep-card');
    if (!card) return;
    if (!state.runtimeDependency) {
    ...
    updateDepSnippet();
    card.querySelectorAll('.runtime-dep-tab').forEach(btn => {
        btn.onclick = () => {
            state.depSnippetTab = btn.dataset.depTab;
            card.querySelectorAll('.runtime-dep-tab').forEach(t =>
                t.classList.toggle('runtime-dep-tab-active', t.dataset.depTab === state.depSnippetTab));
            updateDepSnippet();
        };
    });
    document.getElementById('runtime-dep-copy-btn').onclick = () => {
        const button = document.getElementById('runtime-dep-copy-btn');
        const snippet = depSnippetFor(state.depSnippetTab);
...
function depSnippetFor(tab) {
    const dep = state.runtimeDependency;
    if (!dep) return '';
    return tab === 'maven' ? dep.mavenSnippet : dep.gradleSnippet;
}

function updateDepSnippet() {
    const el = document.getElementById('runtime-dep-snippet');
    if (el) el.textContent = depSnippetFor(state.depSnippetTab);
}

function renderLibrarySources() {
    const panel = document.getElementById('library-sources-panel');
    const content = document.getElementById('library-sources-content');
    ...
    content.innerHTML = state.libraryFiles.map(file => `
        <div class="code-file">
            <div class="code-file-header">
            ...
            <div class="code-preview">
                <pre><code class="language-kotlin">${escapeHtml(file.content)}</code></pre>
            </div>
        </div>
    `).join('');
...
function resetGeneration() {
    state.generatedFiles = [];
    state.libraryFiles = [];
    state.runtimeDependency = null;
    document.getElementById('results-content').innerHTML = '';
    const depCard = document.getElementById('runtime-dep-card');
    if (depCard) depCard.style.display = 'none';
    const libPanel = document.getElementById('library-sources-panel');
    if (libPanel) libPanel.style.display = 'none';
    const libContent = document.getElementById('library-sources-content');
    if (libContent) libContent.innerHTML = '';
    document.getElementById('results-section').style.display = 'none';
    document.getElementById('error-message').style.display = 'none';
